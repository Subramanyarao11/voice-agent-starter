...
from sahaayak_common import Benefit, get_logger, session_scope, settings
...
async def translate(
    client: AsyncOpenAI, text: str, language_name: str, semaphore: asyncio.Semaphore
) -> str:
    async with semaphore:
        response = await client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "user", "content": f"Translate into {language_name}:\n\n{text}"},
            ],
        )
...
async def run(language_codes: list[str], limit: int | None, overwrite: bool) -> None:
    ...
    with session_scope() as db:
        ...
        results = await asyncio.gather(
            *(translate(client, source, p.name, semaphore) for _, p, source in jobs),
